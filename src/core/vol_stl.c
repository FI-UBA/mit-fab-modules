...
int main(int argc, char **argv) {
   ...
   float threshold,gamma,voxel_size;
   ...
   if (!((argc == 6) || (argc == 7) || (argc == 8) || (argc == 9) || (argc == 10))) {
      printf("command line: vol_stl in.vol out.stl nx ny nz [format [threshold [gamma [size [points [angle]]]]]]\n");
      ...
      printf("   gamma: intensity exponent (default 1))\n");
   ...
   if (argc >= 9)
      sscanf(argv[8],"%f",&gamma);
   if (argc >= 10)
      sscanf(argv[9],"%f",&voxel_size);
   if (argc >= 11)
      sscanf(argv[10],"%d",&p);
   //
